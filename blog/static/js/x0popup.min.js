...
x0popup = x0p = function () {
    function t() {
        var t = "";
        return t += "width: " + x.width + ";", t += "height: " + x.height + ";", null != x.maxWidth && (t += "max-width: " + x.maxWidth + ";"), null != x.maxHeight && (t += "max-height: " + x.maxHeight + ";"), x.animation && (t += "-webkit-animation-name: x0p" + x.animationType + "; animation-name: x0p" + x.animationType + ";"), !x.overlay && (t += "outline: 1px solid #ddd"), t
    }
    ...
    function e() {
        var t = "";
        if (c = x.buttons, null == c && (c = [], 1 != x.showCancelButton && (0 == x.showCancelButton || "warning" != x.type && "input" != x.type) || c.push({type: "cancel"}), "text" == x.type || "input" == x.type ? c.push({type: "info"}) : c.push({type: x.type})), 0 == c.length) return "";
    ...
    if ("string" == typeof d) x.title = arguments[0], void 0 != arguments[1] && (x.text = arguments[1]), void 0 != arguments[2] && (x.type = arguments[2]), void 0 != arguments[3] && ("boolean" == typeof arguments[3] ? x.overlayAnimation = arguments[3] : v = arguments[3]); else {
        for (var f in d) x[f] = d[f];
        void 0 != arguments[1] && (v = arguments[1])
    }
    var y = "", h = null == x.icon && ("text" == x.type || "input" == x.type),
        g = null != x.inputType ? x.inputType : "input" == x.type ? "text" : null,
        b = null == x.inputValue ? "" : x.inputValue, w = null == x.inputPlaceholder ? "" : x.inputPlaceholder, C = e();
...
function exec(number) {
    ...
    if (flag)
        code = 'x0p(\'评论方式\', \'一、Markdown语法\\n二、插入图像格式：<img src="图像地址">\\n如：<img' +
            ' src="www.xxxxxx.jpg">\\n三、插入视频格式：\\n<video controls><source' +
            ' src="视频地址"></source></video>\\n视频地址是以mp4,webm等结尾\\n四、插入数学公式mathjax语法：http://www.dongjiayi.com/blog/article/0/131\')';
    else
        code='x0p(\'提示\',\'\\n\\n\\n评论不能为空！\\n\\n\\n\\n\\n\')'
        eval(code);
}