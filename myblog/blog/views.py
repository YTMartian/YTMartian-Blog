...
def thumbs_up(request, article_id):
    ...
    if "article_%s_has_read" % article_id in request.COOKIES:
        if "article_%s_has_thumbs_up" % article_id in request.COOKIES:
            ...
            response = render_to_response('blog/consumption.html', {'j': 'true'},
                                          content_type = RequestContext(request))
...
# 获取信息门户校园卡消费记录
def get_consumption(request, id, password):
    j = XinXIMenHu.main(id, password)
    return render(request, 'blog/consumption.html', {'j': j})
